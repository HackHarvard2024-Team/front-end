...
const NYC_SODA_URL = 'https://data.cityofnewyork.us/resource/5uac-w243.json'
const NYC_START_DATE = '2023-01-01T00:00:00.000'
const METERS_PER_DEGREE_LAT = 111320
const NYC_CLUSTER_RADIUS_MIN_METERS = 35
const NYC_CLUSTER_RADIUS_MAX_METERS = 140
const NYC_CLUSTER_RADIUS_BASE_METERS = 70
const NYC_CLUSTER_ZOOM_REFERENCE = 14
const NYC_CLUSTER_ZOOM_SCALE = 1.2
const NYC_CLUSTER_MIN_POINTS = 1
const NYC_CLUSTER_EXPANSION_MAX = 0.1
const NYC_CLUSTER_SMALL_POLY_SIDES = 6
const NYC_CLUSTER_TOP_OFFENSE_LIMIT = 3
const NYC_VIEW_POINT_LIMIT = 4000
const CACHE_TTL_MS = 30000
const CACHE_MAX_ENTRIES = 60
const OUTPUT_COORD_DECIMALS = 5
const NYC_EXCLUDED_KEYWORDS = [
  'NON-CRIMINAL',
  'NON CRIMINAL',
  'ADMINISTRATIVE',
  'OTHER OFFENSE',
...
const isExcludedOffense = (offenseDescription, offenseGroup, lawCategory) => {
  const text =
    `${offenseDescription || ''} ${offenseGroup || ''} ${lawCategory || ''}`
      .toUpperCase()
      .trim()
  if (!text) {
    return false
  }
  return NYC_EXCLUDED_KEYWORDS.some(keyword => text.includes(keyword))
}
...
const getDangerLevelForOffense = ({
  ...
  const text =
    `${offenseDescription || ''} ${offenseGroup || ''} ${offenseCode || ''}`
      .toUpperCase()
      .trim()
  const lawText = (lawCategory || '').toUpperCase()
  const shootingFlag = String(shooting || '').trim().toLowerCase()

  const matchAny = keywords => keywords.some(keyword => text.includes(keyword))

  if (shootingFlag === '1' || shootingFlag === 'y') {
    return 5
  }
  ...
  if (lawText.includes('FELONY')) {
    return 4
  }
  if (lawText.includes('MISDEMEANOR')) {
    return 3
  }
  if (lawText.includes('VIOLATION')) {
    return 1
  }
...
const fetchNYCPoints = async (bounds, limit) => {
  ...
  const url = new URL(NYC_SODA_URL)
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `NYC Open Data fetch failed: ${response.status} ${text}`,
    )
  }
  ...
  rows.forEach(row => {
    ...
    const offenseGroup = row.ofns_desc?.trim() || ''
    const offenseDescription = row.pd_desc?.trim() || ''
    const offenseCode =
      row.ky_cd !== null && row.ky_cd !== undefined
        ? String(row.ky_cd).trim()
        : ''
    const lawCategory = row.law_cat_cd?.trim() || ''
    const offenseText = `${offenseDescription} ${offenseGroup}`.toUpperCase()
    const shootingFlag = offenseText.includes('SHOOTING') ? '1' : ''

    if (isExcludedOffense(offenseDescription, offenseGroup, lawCategory)) {
      return
    }
    ...
    points.push({
      ...
      offenseGroup,
      occurredOnDate: row.cmplnt_fr_dt || '',
      shooting: shootingFlag,
      dangerLevel,
    })
  })
...
const getNYCClusterRadiusMeters = zoom => {
  if (!Number.isFinite(zoom)) {
    return NYC_CLUSTER_RADIUS_BASE_METERS
  }
  const scale = Math.pow(
    NYC_CLUSTER_ZOOM_SCALE,
    NYC_CLUSTER_ZOOM_REFERENCE - zoom,
  )
  const radius = NYC_CLUSTER_RADIUS_BASE_METERS * scale
  return Math.min(
    NYC_CLUSTER_RADIUS_MAX_METERS,
    Math.max(NYC_CLUSTER_RADIUS_MIN_METERS, radius),
  )
}

const clusterNYCPoints = (points, radiusMeters, referenceLat) => {
  if (!points || points.length === 0) {
    return []
  }
  ...
  for (let i = 0; i < points.length; i += 1) {
    ...
    if (cluster.length >= NYC_CLUSTER_MIN_POINTS) {
...
const clusterToPolygon = (cluster, radiusMeters) => {
  ...
  const expansion =
    1 + Math.min(NYC_CLUSTER_EXPANSION_MAX, Math.log(cluster.length + 1) / 6)
...
const buildClusterDetail = cluster => {
  ...
  const topOffenses = Array.from(counts.entries())
    ...
    .slice(0, NYC_CLUSTER_TOP_OFFENSE_LIMIT)
...
export const handler = async event => {
  ...
  if (event.queryStringParameters?.detailKey) {
    ...
    const limit = Math.min(
      Math.max(parseInteger(cacheParams.limit, NYC_VIEW_POINT_LIMIT), 1),
      NYC_VIEW_POINT_LIMIT,
    )

    let points = null
    try {
      points = await fetchNYCPoints(bounds, limit)
    } catch (error) {
      return jsonResponse(500, { error: 'Failed to load NYC data.' })
    }
    ...
    const radiusMeters = getNYCClusterRadiusMeters(zoom)
    const centerLat = (bounds.north + bounds.south) / 2
    const clusters = clusterNYCPoints(
      filteredPoints,
      radiusMeters,
      centerLat,
    )
  ...
  const limit = Math.min(
    Math.max(
      parseInteger(event.queryStringParameters?.limit, NYC_VIEW_POINT_LIMIT),
      1,
    ),
    NYC_VIEW_POINT_LIMIT,
  )
  ...
  const radiusMeters = getNYCClusterRadiusMeters(zoom)
  const centerLat = (bounds.north + bounds.south) / 2
  const clusters = clusterNYCPoints(filteredPoints, radiusMeters, centerLat)
